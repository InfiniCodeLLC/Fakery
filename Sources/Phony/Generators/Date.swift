...
public extension Phony {
    final class Date {
        public func backward(days: Int) -> Foundation.Date {
            return self.todayAddingDays(-days)
        }
        ...
        public func birthday(_ minAge: Int, _ maxAge: Int) -> Foundation.Date {
            let olderAgeBirthDate = self.todayAddingYears(-maxAge)
            let earlierAgeBirthDate = self.todayAddingYears(-minAge)
            return self.between(earlierAgeBirthDate, olderAgeBirthDate)
        }

        private func todayAddingDays(_ days: Int) -> Foundation.Date {
            var dateComponents = DateComponents()
            dateComponents.day = days
            return self.todayAdding(dateComponents)
        }
